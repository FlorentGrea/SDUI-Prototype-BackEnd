...
fun mapMarker(mapPoint: MapPoint): UIComponent {
    ...
    val imageUrl = if(mapPoint.images.isNotEmpty()) "https://syrpoc.pockethost.io/" + "api/files/" + mapPoint.collectionId + "/" + mapPoint.id + "/" + mapPoint.images[0] else "/placeholder.webp"
                        ...
                        children = listOf(
                            UIComponent(
                                ...
                                properties = mapOf(
                                    ...
                                    "className" to UIProperty.StringValue("absolute top-4 left-4 flex items-center justify-center border-none rounded-full bg-white w-[28px] h-[28px] focus:outline-none shadow-[0px_0px_4px_1px_#00000060]")
                                ),
                                children = listOf(
